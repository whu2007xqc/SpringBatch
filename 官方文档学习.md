...
`JdbcPagingItemReader`通过JDBC的方式分页查询，需要定义数据库对象的映射，定义SQL与入参。
```java
...
`JpaPagingItemReader`使用了Jpa的分页特性，允许使用JPQL语句和EntityManagerFactory。  

使用原始的ItemReader以及数据库的Limit关键字来做分页，以及`MyBatisPagingItemReader`，参考  
